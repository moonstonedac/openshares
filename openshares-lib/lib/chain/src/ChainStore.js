...
var {Apis} = require("openshares-ws");
...
let delegate_object_type  = parseInt(object_type.delegate, 10);
let worker_object_type  = parseInt(object_type.worker, 10);
let council_member_object_type  = parseInt(object_type.council_member, 10);
...
let openasset_data_prefix = "2." + parseInt(impl_object_type.asset_openasset_data,10) + "."
let vesting_balance_prefix = "1." + vesting_balance_type + "."
let delegate_prefix = "1." + delegate_object_type + "."
let worker_prefix = "1." + worker_object_type + "."
let council_prefix = "1." + council_member_object_type + "."
let asset_prefix = "1." + asset_object_type + "."
let account_prefix = "1." + account_object_type + "."

const DEBUG = JSON.parse(process.env.npm_config__capricorn_chain_chain_debug || false)
...
class ChainStore
{
   ...
   clearCache() {
      ...
      this.delegate_by_account_id    = new Map()
      this.council_by_account_id  = new Map()
   ...
   getAsset( id_or_symbol )
   {
      ...
      if ( ChainValidation.is_object_id( id_or_symbol ) ) {
        ...
        if (asset && (asset.get("openasset") && !asset.getIn(["openasset", "current_feed"]))) {
          return undefined;
        }
        return asset;
      }
   ...
   getAccount( name_or_id ) {
    ...
    /**
     * This method will attempt to lookup delegate by account_id.
     * If delegate doesn't exist it will return null, if delegate is found it will return delegate object,
     * if it's not fetched yet it will return undefined.
     * @param account_id - account id
     */
    getDelegateById(account_id) {
        let delegate_id = this.delegate_by_account_id.get(account_id);
        if (delegate_id === undefined) {
            this.fetchDelegateByAccount(account_id);
            return undefined;
        }
        return delegate_id ? this.getObject(delegate_id) : null;
    }

    /**
     * This method will attempt to lookup council member by account_id.
     * If council member doesn't exist it will return null, if council member is found it will return council member object,
     * if it's not fetched yet it will return undefined.
     * @param account_id - account id
     */
    getCouncilMemberById(account_id) {
        let cm_id = this.council_by_account_id.get(account_id);
        if (cm_id === undefined) {
            this.fetchCouncilMemberByAccount(account_id);
            return undefined;
        }
   ...
   /**
    * Obsolete! Please use getDelegateById
    * This method will attempt to lookup the account, and then query to see whether or not there is
    * a delegate for this account.  If the answer is known, it will return the delegate_object, otherwise
    * it will attempt to look it up and return null.   Once the lookup has completed on_update will
    * be called.
    *
    * @param id_or_account may either be an account_id, a delegate_id, or an account_name
    */
   getDelegate( id_or_account )
   {
      ...
      if( delegate_id === undefined )
         this.fetchDelegateByAccount( account_id )
      return this.getObject( delegate_id )
      ...
      if( ChainValidation.is_account_name(id_or_account, true) || (id_or_account.substring(0,4) == "1.2."))
      {
         ...
         if( !account )
         {
            this.lookupAccountByName( id_or_account ).then ( account => {
               ...
               let account_id = account.get('id')
               let delegate_id = this.delegate_by_account_id.get( account_id )
               if( ChainValidation.is_object_id( delegate_id ) )
                   return this.getObject( delegate_id, on_update )

               if( delegate_id == undefined )
                  this.fetchDelegateByAccount( account_id ).then( delegate => {
                         this.delegate_by_account_id.set( account_id, delegate?delegate.get('id'):null )
                         if( delegate && on_update ) on_update()
                  })
            }, error => {
               let delegate_id = this.delegate_by_account_id.set( id_or_account, null )
            } )
         }
         else
         {
    ...
    // Obsolete! Please use getCouncilMemberById
   getCouncilMember( id_or_account, on_update = null )
   {
      if( ChainValidation.is_account_name(id_or_account, true) || (id_or_account.substring(0,4) == "1.2."))
      {
         let account = this.getAccount( id_or_account )
         ...
         if( !account )
         {
            this.lookupAccountByName( id_or_account ).then( account=>{
               let account_id = account.get('id')
               let council_id = this.council_by_account_id.get( account_id )
               if( ChainValidation.is_object_id( council_id ) ) return this.getObject( council_id, on_update )

               if( council_id == undefined )
               {
                  this.fetchCouncilMemberByAccount( account_id ).then( council => {
                       this.council_by_account_id.set( account_id, council ? council.get('id') : null )
                       if( on_update && council) on_update()
                       } )
               }
            }, error => {
               let delegate_id = this.council_by_account_id.set( id_or_account, null )
            })
         }
         else
         {
   ...
   /**
    *
    * @return a promise with the delegate object
    */
   fetchDelegateByAccount( account_id )
   {
      return new Promise( (resolve,reject ) => {
          Apis.instance().db_api().exec( "get_delegate_by_account", [ account_id ] )
              .then( optional_delegate_object => {
                   if( optional_delegate_object )
                   {
                       this.delegate_by_account_id = this.delegate_by_account_id.set( optional_delegate_object.delegate_account, optional_delegate_object.id )
                      let delegate_object = this._updateObject( optional_delegate_object, true )
                      resolve(delegate_object)
                   }
                   else
                   {
                       this.delegate_by_account_id = this.delegate_by_account_id.set( account_id, null )
                       this.notifySubscribers()
                      resolve(null)
                   }
              }, reject ) } )
    ...
    * @return a promise with the delegate object
    */
   fetchCouncilMemberByAccount( account_id )
   {
      return new Promise( (resolve,reject ) => {
          Apis.instance().db_api().exec( "get_council_member_by_account", [ account_id ] )
              .then( optional_council_object => {
                   if( optional_council_object )
                   {
                      this.council_by_account_id = this.council_by_account_id.set( optional_council_object.council_member_account, optional_council_object.id )
                      let council_object = this._updateObject( optional_council_object, true )
                      resolve(council_object)
                   }
                   else
                   {
                       this.council_by_account_id = this.council_by_account_id.set( account_id, null )
                       this.notifySubscribers()
                      resolve(null)
                   }
              }, reject ) } )
   ...
   _updateObject( object, notify_subscribers, emit = true )
   {
      ...
      // DELEGATE OBJECT
      else if( object.id.substring(0,delegate_prefix.length) == delegate_prefix )
      {
         this.delegate_by_account_id.set( object.delegate_account, object.id )
         this.objects_by_vote_id.set( object.vote_id, object.id )
      }
      // COUNCIL MEMBER OBJECT
      else if( object.id.substring(0,council_prefix.length) == council_prefix )
      {
         this.council_by_account_id.set( object.council_member_account, object.id )
         this.objects_by_vote_id.set( object.vote_id, object.id )
      }
      // ACCOUNT OBJECT
      ...
      else if( object.id.substring(0,asset_prefix.length) == asset_prefix )
      {
         ...
         let openasset = current.get( 'openasset' );
         if( !openasset && object.openasset_data_id ) {
            let bad = this.getObject( object.openasset_data_id, true );
            if( !bad )
               bad = Immutable.Map()
            ...
            this.objects_by_id = this.objects_by_id.set( object.openasset_data_id, bad );

            current = current.set( 'openasset', bad );
            this.objects_by_id = this.objects_by_id.set( object.id, current );
         }
      }
      ...
      // OPENASSET DATA OBJECT
      else if( object.id.substring(0,openasset_data_prefix.length) == openasset_data_prefix )
      {
          let asset_id = current.get( "asset_id" );
          if( asset_id ) {
             let asset = this.getObject( asset_id );
             if( asset ) {
                asset = asset.set( "openasset", current );
                emitter.emit('openasset-update', asset);
...
function timeStringToDate(time_string) {
    ...
    if( ! /Z$/.test(time_string)) //does not end in Z
        // https://github.com/cryptonomex/capricorn/issues/368
