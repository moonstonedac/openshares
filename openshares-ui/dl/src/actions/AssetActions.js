...
class AssetActions {
    ...
    createAsset(account_id, createObject, flags, permissions, cer, isOpenAsset, is_prediction_market, openasset_opts, description) {
        // Create asset action here...
        console.log("create asset:", createObject, "flags:", flags, "isOpenAsset:", isOpenAsset, "openasset_opts:", openasset_opts);
        let tr = wallet_api.new_transaction();
        ...
        if (isOpenAsset) {
            operationJSON.openasset_opts = openasset_opts;
        }
    ...
    updateAsset(issuer, new_issuer, update, core_exchange_rate, asset, flags, permissions,
            isOpenAsset, openasset_opts, original_openasset_opts, description) {
        ...
        console.log("openasset_opts:", openasset_opts, "original_openasset_opts:", original_openasset_opts);
        if (isOpenAsset &&
            (   openasset_opts.feed_lifetime_sec !== original_openasset_opts.feed_lifetime_sec ||
                openasset_opts.minimum_feeds !== original_openasset_opts.minimum_feeds ||
                openasset_opts.force_settlement_delay_sec !== original_openasset_opts.force_settlement_delay_sec ||
                openasset_opts.force_settlement_offset_percent !== original_openasset_opts.force_settlement_offset_percent ||
                openasset_opts.maximum_force_settlement_volume !== original_openasset_opts.maximum_force_settlement_volume ||
                openasset_opts.short_backing_asset !== original_openasset_opts.short_backing_asset)) {

            let openAssetUpdateObject = {
                fee: {
                    amount: 0,
                    asset_id: 0
                },
                asset_to_update: asset.get("id"),
                issuer: issuer,
                new_options: openasset_opts
            }

            tr.add_type_operation("asset_update_openasset", openAssetUpdateObject);
        ...
        if (!inProgress[id]) {
            ...
            Apis.instance().db_api().exec("list_assets", [
                    ...
                    assets.forEach(asset => {
                        ...
                        if (asset.openasset_data_id) {
                            openAssetIDS.push(asset.openasset_data_id);
                        }
                    });
                    ...
                    let openAssetPromise = openAssetIDS.length > 0 ? Apis.instance().db_api().exec("get_objects", [
                        openAssetIDS
                    ]) : null;
    ...
    getAsset(id) {
        ...
        if (!inProgress[id]) {
            ...
            return assetPromise.then((asset) => {
                ...
                let openAssetPromise = asset[0].openasset_data_id ? Apis.instance().db_api().exec("get_objects", [
                    [asset[0].openasset_data_id]
                ]) : null;
                ...
                Promise.all([
                    ...
                    .then(results => {
                        this.dispatch({
                            ...
                            openasset_data: results[1] ? results[1][0] : null
                        });
                        delete inProgress[id];
                    });
