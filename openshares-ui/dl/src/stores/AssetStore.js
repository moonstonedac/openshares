...
class AssetStore extends BaseStore {
    ...
    onGetAssetList(payload) {
        ...
        payload.assets.forEach(asset => {
            ...
            if (asset.openasset_data_id) {
                ...
                for (var i = 0; i < payload.openasset_data.length; i++) {
                    if (payload.openasset_data[i].id === asset.openasset_data_id) {
                        asset.openasset_data = payload.openasset_data[i];
                        break;
                    }
                }
            } else {
    ...
    onGetAsset(payload) {
        ...
        if (payload.openasset_data) {
            asset.openasset_data = payload.openasset_data;
            asset.market_asset = true;
        } else {
            asset.market_asset = false;
        }
