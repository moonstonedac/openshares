...
// Filter OPS 0.9.2+ import keys export file so that it will include only private keys
// that may be found in the OPS 2.0 genesis block. 

// Dependencies:
// ./bloom.dat (1,048,576 bytes) sha1 3cee441d8d28ab3b26aea149630fa2a96a91845c
// nodejs, npm, and: npm install coffee-script

// ./bloom.dat is from openshares/capricorn/programs/genesis_util/create_bloom_filter.py
// The bloom filter was created with a genesis containing OPS prefixed keys.  Create
...
chain_config.address_prefix = "OPS"
...
fs.readFile('bloom.dat', function (err, data) {
    ...
    function in_bloom(str) {
        // echo -n '0:OPS87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3m'|sha256sum
        // str = 'OPS87mopaNqLDjT1BvzqQR3QjWzWSTgkWnMcwt5sqxHuavCBi1s3n' // should be found
        // str = 'OPS4A43UCoWz1F5vqLxX3LLoGHKs1GzCGRjZ' // should be found
    ...
    stdin.on('end', function () {
        ...
        for(var a = 0; a < wallet.account_keys.length; a++) {
            ...
            for(var k = keys.encrypted_private_keys.length - 1; k >= 0; k--) {
                ...
                if( /^CAP/.test(key) ) key = "OPS" + key.substring(3)
