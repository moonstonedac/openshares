...
function getDelegateOrCouncil(type, acct) {
    let url = "", votes = 0, account
    if (type === "delegate") {
        account = ChainStore.getDelegateById(acct.get("id"));
    } else if (type === "council") {
        account = ChainStore.getCouncilMemberById(acct.get("id"));
    }
...
class AccountItemRow extends React.Component {
    ...
    render() {
        ...
        let {url, votes} = getDelegateOrCouncil(type, account);
            ...
            <tr>
                ...
                <td>
                    <button className="button outline" onClick={this.onAction.bind(this, item_id)}>
                        <Translate content={`account.votes.${this.props.action}_delegate`}/></button>
                </td>
            </tr>
        );
    }
...
class AccountsList extends React.Component {
    ...
    render() {
        ...
        .sort((a,b) =>{
            let {votes: a_votes} = getDelegateOrCouncil(this.props.type, a);
            let {votes: b_votes} = getDelegateOrCouncil(this.props.type, b);
            if (a_votes !== b_votes) {
        ...
        return (
            <div>
                ...
                <AccountSelector
                    ...
                    action_label="account.votes.add_delegate"
