...
var Utils = {
    ...
    convertPrice: function(fromRate, toRate, fromID, toID) {
        ...
        if (fromRate.toJS && this.is_object_type(fromRate.get("id"), "asset")) {
            ...
            fromRate = fromRate.get("openasset") ? fromRate.getIn(["openasset", "current_feed", "settlement_price"]).toJS() : fromRate.getIn(["options", "core_exchange_rate"]).toJS();
        }

        if (toRate.toJS && this.is_object_type(toRate.get("id"), "asset")) {
            toID = toRate.get("id");
            toRate = toRate.get("openasset") ? toRate.getIn(["openasset", "current_feed", "settlement_price"]).toJS() : toRate.getIn(["options", "core_exchange_rate"]).toJS();
        }
    ...
    replaceName(name, isOpenAsset = false) {
        ...
        return {
            name,
            prefix: isOpenAsset ? "bit" : toReplace[i] ? toReplace[i].toLowerCase() : null
        };
    }
};
