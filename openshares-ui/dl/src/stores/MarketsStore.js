...
let marketStorage = new ls("__capricorn__");
...
class MarketsStore {
    ...
    constructCalls (callsArray) {
        ...
        if (this.activeMarketCalls.size) {
            ...
            if (this.invertedCalls) {
                squeezeRatio = this.baseAsset.getIn(["openasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;
                maintenanceRatio = this.baseAsset.getIn(["openasset", "current_feed", "maintenance_collateral_ratio"]) / 1000;
                settlementPrice = market_utils.getFeedPrice(
                    this.baseAsset.getIn(["openasset", "current_feed", "settlement_price"]),
                    true
                )
                 this.lowestCallPrice = settlementPrice / 5;
            } else {
                squeezeRatio = this.quoteAsset.getIn(["openasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;
                maintenanceRatio = this.quoteAsset.getIn(["openasset", "current_feed", "maintenance_collateral_ratio"]) / 1000;
                settlementPrice = market_utils.getFeedPrice(
                    this.quoteAsset.getIn(["openasset", "current_feed", "settlement_price"]),
                    false
                )
                this.lowestCallPrice = settlementPrice * 5;
            }
        ...
        callsArray.filter(a => {
            ...
            if (this.invertedCalls) {
                let newQuote = this.baseAsset.getIn(["openasset", "current_feed", "settlement_price", "base"]).toJS();
                newQuote.amount /= squeezeRatio;

                feed_price_order = {
                    call_price: {
                        base: this.baseAsset.getIn(["openasset", "current_feed", "settlement_price", "quote"]).toJS(),
                        quote: newQuote
                    },
                    debt: order.debt,
                ...
                let newQuote = this.quoteAsset.getIn(["openasset", "current_feed", "settlement_price", "quote"]).toJS();
                newQuote.amount *= squeezeRatio;
                feed_price_order = {
                    sell_price: {
                        base: this.quoteAsset.getIn(["openasset", "current_feed", "settlement_price", "base"]).toJS(),
                        quote: newQuote
                    },
                    debt: order.debt,
                    collateral: order.collateral
