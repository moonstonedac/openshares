...
import {ChainStore, ChainTypes} from "openshares-lib";
...
class MarketUtils {
    ...
    static parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/OPS, amount should be in OPS, value should be in USD, debt is in USD
                // buy is in USD, sell is in OPS
                // quote is USD, base is OPS
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in OPS/USD, amount should be in USD, value should be in OPS, debt is in USD
                // buy is in USD, sell is in OPS
                // quote is USD, base is OPS
    ...
    static isMarketAsset(quote, base) {
        ...
        if (quote.get("openasset") && base.get("id") === quote.getIn(["openasset", "options", "short_backing_asset"])) {
            isMarketAsset = true;
            marketAsset = {id: quote.get("id")}
        } else if (base.get("openasset") && quote.get("id") === base.getIn(["openasset", "options", "short_backing_asset"])) {
            inverted = true;
            isMarketAsset = true;
