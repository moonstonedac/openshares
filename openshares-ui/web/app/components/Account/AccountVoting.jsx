...
import {ChainStore, FetchChainObjects} from "openshares-lib";
...
class AccountVoting extends React.Component {
    ...
    updateAccountData(account) {
        ...
        FetchChainObjects(ChainStore.getObjectByVoteID, vote_ids, 5000).then(vote_objs => {
            ...
            let delegates = new Immutable.List();
            let council = new Immutable.List();
            let workers = new Immutable.Set();
            vote_objs.forEach( obj => {
                let account_id = obj.get("council_member_account");
                if (account_id) {
                    council = council.push(account_id);
                } else if( account_id = obj.get( "worker_account" ) ) {
                   // console.log( "worker: ", obj );
               //     workers = workers.add(obj.get("id"));
                } else if( account_id = obj.get("delegate_account") ) {
                    delegates = delegates.push(account_id);
                }
            });
            let state = {
    ...
    isChanged() {
        ...
        return s.proxy_account_id !== s.prev_proxy_account_id ||
               s.delegates !== s.prev_delegates ||
               s.council !== s.prev_council ||
    ...
    onPublish() {
        ...
        updated_account.new_options.num_delegate = this.state.delegates.size;
        updated_account.new_options.num_council = this.state.council.size;
        ...
        // Submit votes 
        FetchChainObjects(ChainStore.getDelegateById, this.state.delegates.toArray(), 4000).then( res => {
            let delegates_vote_ids = res.map(o => o.get("vote_id"));
            return Promise.all([Promise.resolve(delegates_vote_ids), FetchChainObjects(ChainStore.getCouncilMemberById, this.state.council.toArray(), 4000)]);
        }).then( res => {
    ...
    onReset() {
        ...
        this.setState({
            ...
            delegates: s.prev_delegates,
            council: s.prev_council,
    ...
    validateAccount(collection, account) {
        ...
        if(collection === "delegates") {
            return FetchChainObjects(ChainStore.getDelegateById, [account.get("id")], 3000).then(res => {
                return res[0] ? null : "Not a delegate";
            });
        }
        if(collection === "council") {
            return FetchChainObjects(ChainStore.getCouncilMemberById, [account.get("id")], 3000).then(res => {
                return res[0] ? null : "Not a council member";
            });
        }
        return null;
    }
    ...
    render() {
        ...
        let unVotedActiveDelegates = globalObject.get("active_delegates").map(a => {
            let object = ChainStore.getObject(a);
            if (!object || !this.state.delegates) {
                return null;
            }
            if (!this.state.delegates.includes(object.get("delegate_account"))) {
                return object.get("delegate_account");
            } else {
                return null;
            }
        }).filter(a => {
        ...
        let unVotedActiveCMs = globalObject.get("active_council_members").map(a => {
            let object = ChainStore.getObject(a);
            if (!object || !this.state.council) {
                return null;
            }
            if (!this.state.council.includes(object.get("council_member_account"))) {
                return object.get("council_member_account");
            } else {
                return null;
            }
        }).filter(a => {
        ...
        return (
            <div className="grid-content">
                ...
                <Tabs setting="votingTab" tabsClass="no-padding bordered-header" contentClass="grid-content">
                        ...
                        <Tab title="explorer.delegates.title">
                            <div className={cnames("content-block", {disabled : proxy_is_set})}>
                                <HelpContent style={{maxWidth: "800px"}} path="components/AccountVotingDelegates" />
                                <AccountsList
                                    type="delegate"
                                    label="account.votes.add_delegate_label"
                                    items={this.state.delegates}
                                    validateAccount={this.validateAccount.bind(this, "delegates")}
                                    onAddItem={this.onAddItem.bind(this, "delegates")}
                                    onRemoveItem={this.onRemoveItem.bind(this, "delegates")}
                                    tabIndex={proxy_is_set ? -1 : 2}
                                    title={counterpart.translate("account.votes.w_approved_by", {account: this.props.account.get("name")})}
                                />

                                {unVotedActiveDelegates.size ? (
                                <AccountsList
                                    type="delegate"
                                    label="account.votes.add_delegate_label"
                                    items={Immutable.List(unVotedActiveDelegates)}
                                    validateAccount={this.validateAccount.bind(this, "delegates")}
                                    onAddItem={this.onAddItem.bind(this, "delegates")}
                                    onRemoveItem={this.onRemoveItem.bind(this, "delegates")}
                                    tabIndex={proxy_is_set ? -1 : 2}
                        ...
                        <Tab title="explorer.council_members.title">
                            <div className={cnames("content-block", {disabled : proxy_is_set})}>
                                <HelpContent style={{maxWidth: "800px"}} path="components/AccountVotingCouncil" />
                                <AccountsList
                                    type="council"
                                    label="account.votes.add_council_label"
                                    items={this.state.council}
                                    validateAccount={this.validateAccount.bind(this, "council")}
                                    onAddItem={this.onAddItem.bind(this, "council")}
                                    onRemoveItem={this.onRemoveItem.bind(this, "council")}
