...
import {ChainStore, EmitterInstance} from "openshares-lib";
...
class Exchange extends React.Component {
    ...
    componentWillMount() {
        ...
        emitter.on('openasset-update', feedUpdateListener = MarketsActions.feedUpdate);
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.baseAsset && nextProps.baseAsset.getIn(["openasset", "is_prediction_market"])) {
    ...
    componentWillUnmount() {
        ...
        if (emitter) {
            ...
            emitter.off('openasset-update', feedUpdateListener);
    }
    ...
    _createLimitOrderConfirm(buyAsset, sellAsset, buyAssetAmount, sellAssetAmount, sellBalance, coreBalance, feeAsset, type, short = true, e) {
        ...
        let isPredictionMarket = sellAsset.getIn(["openasset", "is_prediction_market"]);
    ...
    _getSettlementInfo() {
        ...
        if (isMarketAsset && quote && base) {
            if (quote.get("openasset") && quote.getIn(["openasset", "current_feed"]) && base.get("id") === quote.getIn(["openasset", "options", "short_backing_asset"])) {
                settlement_price = quote.getIn(["openasset", "current_feed", "settlement_price"]);
                short_squeeze = quote.getIn(["openasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;

            } else if (base.get("openasset") && base.getIn(["openasset", "current_feed"]) && quote.get("id") === base.getIn(["openasset", "options", "short_backing_asset"])) {
                settlement_price = base.getIn(["openasset", "current_feed", "settlement_price"]);
                short_squeeze = base.getIn(["openasset", "current_feed", "maximum_short_squeeze_ratio"]) / 1000;
            }
    ...
    render() {
        ...
        let quoteIsOpenAsset = quoteAsset.get("openasset_data_id") ? true : false;
        let baseIsOpenAsset = baseAsset.get("openasset_data_id") ? true : false;
        ...
        let hasPrediction = base.getIn(["openasset", "is_prediction_market"]) || quote.getIn(["openasset", "is_prediction_market"]);
        ...
        let buyForm = (
            <BuySell
                ...
                isPredictionMarket={base.getIn(["openasset", "is_prediction_market"])}
        ...
        let sellForm = (
            <BuySell
                ...
                isPredictionMarket={quote.getIn(["openasset", "is_prediction_market"])}
                onFlip={this.state.flipBuySell ? this._flipBuySell.bind(this) : null}
            />
        );
                ...
                <div className="grid-block page-layout market-layout">
                                ...
                                <div className="grid-block vertical">
                                    ...
                                    <div className="grid-block wrap no-overflow" style={{justifyContent: "space-between"}}>
                                        <ul className="market-stats stats bottom-stats">
                                         ...
                                         <ul className="market-stats stats bottom-stats">
                                            {!isNullAccount && quoteIsOpenAsset ?
                    ...
                    {!isNullAccount && quoteIsOpenAsset  ?
                        <BorrowModal
                            ...
                            backing_asset={quoteAsset.getIn(["openasset", "options", "short_backing_asset"])}
                    ...
                    {!isNullAccount && baseIsOpenAsset ?
                        <BorrowModal
                            ...
                            backing_asset={baseAsset.getIn(["openasset", "options", "short_backing_asset"])}
                            account={currentAccount}
                        /> : null}
