...
class Asset extends React.Component {
    ...
    _assetType(asset) {
        return ('openasset' in asset) ?
               (asset.openasset.is_prediction_market ? 'Prediction' : 'Smart') :
               'Simple';
    }
    ...
    renderAboutBox(asset) {
        ...
        let preferredMarket = description.market ? description.market : core_asset ? core_asset.get("symbol") : "OPS";
        ...
        let {name, prefix} = utils.replaceName(asset.symbol, "openasset" in asset && !asset.openasset.is_prediction_market && asset.issuer === "1.2.0");
    ...
    renderSummary(asset) {
        ...
        let flagBooleans = assetUtils.getFlagBooleans(asset.options.flags, this.props.asset.has("openasset_data_id"));
    ...
    renderPriceFeed(asset) {
        ...
        var openAsset = asset.openasset;
        if (!('current_feed' in openAsset))
            return ( <div header= {title} /> );
        var currentFeed = openAsset.current_feed;
    ...
    renderPermissions(asset) {
        ...
        let permissionBooleans = assetUtils.getFlagBooleans(asset.options.issuer_permissions, this.props.asset.has("openasset_data_id"));
    ...
    renderPriceFeedData(asset) {
        ...
        var openAsset = asset.openasset;
        if (!('feeds' in openAsset) || openAsset.feeds.length == 0 || openAsset.is_prediction_market) {
            return null;
        }

        let now = new Date().getTime();
        let oldestValidDate = new Date(now - asset.openasset.options.feed_lifetime_sec * 1000);

        // Filter by valid feed lifetime, Sort by published date
        var feeds = openAsset.feeds;
        feeds = feeds
        .filter(a => {
    ...
    render()
    {
        ...
        var priceFeed = ('openasset' in asset) ? this.renderPriceFeed(asset) : null;
        var priceFeedData = ('openasset' in asset) ? this.renderPriceFeedData(asset) : null;
