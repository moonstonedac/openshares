...
import {ChainStore, ChainValidation} from "openshares-lib";
...
@BindToChainState()
class OpenAssetOptions extends React.Component {
    ...
    _onFoundBackingAsset(asset) {
        if (asset) {
            if (asset.get("id") === "1.3.0" || (asset.get("openasset_data_id") && !asset.getIn(["openasset", "is_prediction_market"]))) {       
    ...
    render() {
        let {openasset_opts} = this.props;
        let {error} = this.state;

        return (
            <div>
                <label><Translate content="account.user_issued_assets.feed_lifetime_sec" />
                    <input type="number" value={openasset_opts.feed_lifetime_sec / 60} onChange={this.props.onUpdate.bind(this, "feed_lifetime_sec")}/>
                </label>

                <label><Translate content="account.user_issued_assets.minimum_feeds" />
                    <input type="number" value={openasset_opts.minimum_feeds} onChange={this.props.onUpdate.bind(this, "minimum_feeds")}/>
                </label>

                <label><Translate content="account.user_issued_assets.force_settlement_delay_sec" />
                    <input type="number" value={openasset_opts.force_settlement_delay_sec / 60} onChange={this.props.onUpdate.bind(this, "force_settlement_delay_sec")}/>
                </label>

                <label><Translate content="account.user_issued_assets.force_settlement_offset_percent" />
                    <input type="number" value={openasset_opts.force_settlement_offset_percent / assetConstants.CAPRICORN_1_PERCENT} onChange={this.props.onUpdate.bind(this, "force_settlement_offset_percent")}/>
                </label>

                <label><Translate content="account.user_issued_assets.maximum_force_settlement_volume" />
                    <input type="number" value={openasset_opts.maximum_force_settlement_volume / assetConstants.CAPRICORN_1_PERCENT} onChange={this.props.onUpdate.bind(this, "maximum_force_settlement_volume")}/>
                </label>
...
class AccountAssetCreate extends React.Component {
    ...
    resetState(props) {
        ...
        let isOpenAsset = false;
        let precision = utils.get_asset_precision(4);
        let corePrecision = utils.get_asset_precision(props.core.get("precision"));

        let {flagBooleans, permissionBooleans} = this._getPermissions({isOpenAsset});
        
        // let flags = assetUtils.getFlags(flagBooleans);
        // let permissions = assetUtils.getPermissions(permissionBooleans, isOpenAsset);
        ...
        return {
            ...
            openasset_opts: {
                ...
                "force_settlement_offset_percent" : 1 * assetConstants.CAPRICORN_1_PERCENT,
                "maximum_force_settlement_volume" : 20 * assetConstants.CAPRICORN_1_PERCENT,
    ...
    _getPermissions(state) {
        let flagBooleans = assetUtils.getFlagBooleans(0, state.isOpenAsset);
        let permissionBooleans = assetUtils.getFlagBooleans("all", state.isOpenAsset);
    ...
    _createAsset(e) {
        ...
        let {update, flagBooleans, permissionBooleans, core_exchange_rate,
            isOpenAsset, is_prediction_market, openasset_opts} = this.state;
        
        let {account} = this.props;

        let flags = assetUtils.getFlags(flagBooleans, isOpenAsset);
        let permissions = assetUtils.getPermissions(permissionBooleans, isOpenAsset);
        ...
        AssetActions.createAsset(account.get("id"), update, flags, permissions, core_exchange_rate, isOpenAsset, is_prediction_market, openasset_opts, description).then(result => {
    ...
    onChangeOpenAssetOpts(value, e) {
        let {openasset_opts} = this.state;

        switch (value) {
            case "force_settlement_offset_percent":
            case "maximum_force_settlement_volume":
                openasset_opts[value] = parseFloat(e.target.value) * assetConstants.CAPRICORN_1_PERCENT;
                break;

            case "feed_lifetime_sec":
            case "force_settlement_delay_sec":
                console.log(e.target.value, parseInt(parseFloat(e.target.value) * 60, 10));
                openasset_opts[value] = parseInt(parseFloat(e.target.value) * 60, 10);
                break;

            case "short_backing_asset":
                openasset_opts[value] = e;
                break;

            default:
                openasset_opts[value] = e.target.value;
                break;
        }
    ...
    _onToggleOpenAsset() {
        let {update} = this.state;
        this.state.isOpenAsset = !this.state.isOpenAsset;
        if (!this.state.isOpenAsset) {
    ...
    render() {
        ...
        let {errors, isValid, update, assets, flagBooleans, permissionBooleans,
            core_exchange_rate, is_prediction_market, isOpenAsset, openasset_opts} = this.state;
            ...
            <div className="grid-block">
                <div className="grid-content">
                    ...
                    <Tabs setting="createAssetTab" style={{maxWidth: "800px"}} contentClass="grid-block shrink small-vertical medium-horizontal">
                                ...
                                <table className="table" style={{width: "inherit"}}>
                                    <tbody>
                                        <tr>
                                            ...
                                            <td style={{border: "none"}}>
                                                <div className="switch" style={{marginBottom: "10px"}} onClick={this._onToggleOpenAsset.bind(this)}>
                                                    <input type="checkbox" checked={isOpenAsset} />
                                                    <label />
                                                </div>
                                            </td>
                                        </tr>
                        ...
                        {isOpenAsset ? (
                            <Tab title="account.user_issued_assets.openasset_opts">
                                <OpenAssetOptions
                                    openasset_opts={openasset_opts}
                                    onUpdate={this.onChangeOpenAssetOpts.bind(this)}
                                    backingAsset={openasset_opts.short_backing_asset}
...
AccountAssetCreate.OpenAssetOptions = OpenAssetOptions;
