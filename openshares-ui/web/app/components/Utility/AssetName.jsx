...
class AssetName extends React.Component {
	...
	render() {
		...
		let isOpenAsset = asset.has("openasset");
		let isPredMarket = isOpenAsset && asset.getIn(["openasset", "is_prediction_market"]);

		let {name: replacedName, prefix} = utils.replaceName(name, isOpenAsset && !isPredMarket && asset.get("issuer") === "1.2.0");
		// let prefix = isOpenAsset && !isPredMarket ? <span>bit</span> :
